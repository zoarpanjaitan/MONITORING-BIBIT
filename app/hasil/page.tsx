        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-4 flex justify-between items-center relative z-10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <h1 className="text-base font-black text-slate-900">Arsip Laporan Harian</h1>
            </div>
          </div>
          <Link href="/" className="text-xs font-bold text-emerald-700 hover:text-emerald-800 bg-emerald-50 px-4 py-2.5 rounded-2xl transition border border-emerald-100">
            ← Kembali ke Beranda
          </Link>
        </div>
      </header>

      {/* Main Content (Container Diperlebar ke 1440px) */}
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 pt-10 space-y-10 relative z-10">
        ...
        <div className="bg-white/90 backdrop-blur-xl p-8 rounded-3xl border border-white shadow-xl shadow-slate-900/5">
          <div className="mb-6">
            <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider">Executive Summary</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {Object.keys(TARGET_BIBIT_RW).map((rwKey) => {
              ...
              let score = latest && totalTarget > 0 ? Math.min(100, Math.round(((Number(latest.rosella_baik) + Number(latest.sambung_nyawa_baik) + Number(latest.binahong_baik) + Number(latest.ubi_baik)) / totalTarget) * 100)) : 0;
              return (
        ...
        <div>
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-500 mt-1">Klik pada salah satu tanggal laporan untuk membuka rincian data lengkap.</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {uniqueDates.map((tgl) => (
                <Link key={tgl} href={`/hasil/${tgl}`} className="group bg-white/90 backdrop-blur-xl p-6 rounded-3xl border border-white shadow-sm hover:border-emerald-500 hover:shadow-xl transition-all flex items-center justify-between">
                  <div className="flex items-center space-x-5">
                    <div className="w-14 h-14 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-black text-xl group-hover:scale-110 transition">📅</div>
                    <div>
                      <h3 className="text-lg font-black text-slate-900 group-hover:text-emerald-600 transition">Tanggal {tgl}</h3>
                      <p className="text-sm text-slate-400 mt-0.5">Rincian data per-RW</p>
                    </div>
                  </div>
                  <span className="bg-slate-900 text-white group-hover:bg-emerald-600 font-bold text-sm px-6 py-3 rounded-xl transition">Buka</span>
                </Link>
              ))}
            </div>
          )}
